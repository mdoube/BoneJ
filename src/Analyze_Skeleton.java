...
/**
 * Main class. This class is a plugin for the ImageJ interface for analyzing
...
public class Analyze_Skeleton implements PlugInFilter {
	...
	public void run(ImageProcessor ip) {
		...
		if (doPrune) {
			...
			ImagePlus taggedIP = new ImagePlus("Tagged Pruned Image",
					this.taggedImage);
	...
	private void showResults() {
		...
		for (int i = 0; i < this.numOfTrees; i++) {
			...
			ri.setResultInRow(this.imRef, "Junctions",
					this.listOfSingleJunctions[i].size());
			ri.setResultInRow(this.imRef, "End Points", this.endPointsTree[i]
					.size());
			ri.setResultInRow(this.imRef, "Junction Voxels",
					this.junctionVoxelTree[i].size());
			ri.setResultInRow(this.imRef, "Triple Points",
					this.numberOfTriplePoints[i]);
			ri.setResultInRow(this.imRef, "Slab Voxels", this.numberOfSlabs[i]);
			ri.setResultInRow(this.imRef, "Mean Branch Length (" + unit + ")",
					this.averageBranchLength[i]);
			ri.setResultInRow(this.imRef, "Max Branch Length (" + unit + ")",
					this.maximumBranchLength[i]);
			ri.setResultInRow(this.imRef, "Total Branch Length (" + unit + ")",
					this.branchLength[i]);
		}
		ri.updateTable();
	}
	...
	private void visitSkeleton(ImageStack taggedImage, ImageStack treeImage,
		...
		for (int j = 0; j < this.listOfSingleJunctions[iTree].size(); j++) {
			ArrayList<int[]> groupOfJunctions = this.listOfSingleJunctions[iTree]
					.get(j);

			// work out the junction's centroid
			int[] sumJunc = { 0, 0, 0 }; // sum of junction coordinates
			double[] centroidJunc = { 0, 0, 0 }; // centroid of junction
		...
		if (this.numberOfBranches[iTree] == 0)
			return;
		// Average length
		this.averageBranchLength[iTree] = this.branchLength[iTree]
				/ this.numberOfBranches[iTree];
	...
	private ImageStack markTrees(ImageStack taggedImage) {
		...
		while (iteri.hasNext()) {
			...
			if (isVisited(p) == false) {
				...
				// else, visit branch until next junction, end point or visited
				// point
	...
	private int visitTree(int[] startingPoint, ImageStack outputImage,
		...
		// Set pixel color
		this.setPixel(outputImage, startingPoint[0], startingPoint[1],
				startingPoint[2], color);
		...
		while (nextPoint != null || toRevisit.size() != 0) {
			if (nextPoint != null) {
				if (!isVisited(nextPoint)) {
					...
					this.setPixel(outputImage, nextPoint[0], nextPoint[1],
							nextPoint[2], color);
	...
	private double visitBranch(int[] startingPoint, int iTree) {
		...
		if (isSlab(startingPoint) && isSlab(nextPoint)) {
			...
			search: for (int zn = z - 1; zn <= z + 1; zn++) {
				for (int yn = y - 1; yn <= y + 1; yn++) {
					for (int xn = x - 1; xn <= x + 1; xn++) {
						if (xn == x && yn == y && zn == z)
							continue;
						else if (xn == nextPoint[0] && yn == nextPoint[1]
								&& zn == nextPoint[2])
							continue;
						int[] p = { xn, yn, zn };
						if (isSlab(p)) {
			...
			if (getPixel(this.taggedImage, nextPoint) == Analyze_Skeleton.JUNCTION) {
				...
				for (int j = 0; j < this.listOfSingleJunctions[iTree].size(); j++) {
					...
					for (int k = 0; k < groupOfJunctions.size(); k++) {
						...
						if (ka[0] == nextPoint[0] && ka[1] == nextPoint[1]
								&& ka[2] == nextPoint[2]) {
							...
							double[] junctionCentroid = { sumX / nJVox,
									sumY / nJVox, sumZ / nJVox };
							length += calculateDistance(nextPoint,
									junctionCentroid);
							this.auxPoint = previousPoint;
							return length;
						}
	...
	private void fusionNeighborJunction(int[] startingPoint,
		...
		while (nextPoint != null || toRevisit.size() != 0) {
			...
			{
				nextPoint = toRevisit.get(0);
				// IJ.log("visiting " + pointToString(nextPoint)+ " color = " +
				// color);
	...
	private void calculateTriplePoints() {
		for (int iTree = 0; iTree < this.numOfTrees; iTree++) {
			...
			for (int i = 0; i < this.listOfSingleJunctions[iTree].size(); i++) {
				...
				// Count the number of slab neighbors of every voxel in the
				// group
				int nSlab = 0;
				for (int j = 0; j < groupOfJunctions.size(); j++) {
					...
					byte[] neighborhood = this.getNeighborhood(
							this.taggedImage, pj[0], pj[1], pj[2]);
					for (int k = 0; k < 27; k++)
						if (neighborhood[k] == Analyze_Skeleton.SLAB)
							nSlab++;
				}
				// If the junction has only 3 slab neighbors, then it is a
				// triple point
				if (nSlab == 3)
	...
	/**
	 * Get next unvisited neighbor voxel Modified to prevent traversing from one
	 * junction voxel to another
	 * 
	 * @param point
	 *            starting point
	 * @param isJunction
	 *            true if the starting point is a junction voxel
	 * @return unvisited neighbor or null if all neighbors are visited
	 */
		...
		for (int x = -1; x < 2; x++)
			for (int y = -1; y < 2; y++)
				for (int z = -1; z < 2; z++) {
					if (x == 0 && y == 0 && z == 0)
						continue;
					byte pixel = getPixel(this.taggedImage, xp + x, yp + y, zp
							+ z);
		...
		for (int x = -1; x < 2; x++)
			for (int y = -1; y < 2; y++)
				for (int z = -1; z < 2; z++) {
					...
					if (getPixel(this.inputImage, point[0] + x, point[1] + y,
							point[2] + z) != 0
							&& isVisited(point[0] + x, point[1] + y, point[2]
									+ z) == false
							&& isJunction(point[0] + x, point[1] + y, point[2]
									+ z)) {
						unvisitedNeighbor = new int[] { point[0] + x,
								point[1] + y, point[2] + z };
						break;
					}
	 ...
	 * 
	 * @param stack
	 *            ImageStack tagged skeleton image
	 * 
	 */
	private ImageStack pruneEndBranches(ImageStack stack) {
		...
		prune: while (!this.listOfEndPoints.isEmpty()) {
			...
			while (iteri.hasNext()) {
				...
				while (iterk.hasNext()) {
					...
					if (junctionVoxel[0] == x && junctionVoxel[1] == y
							&& junctionVoxel[2] == z) {
						iteri.remove(); // note this is iteri not iterk
						// Check if point is Euler invariant, simple and not an
						// endpoint
				...
				// if the endPoint has only one neighbour, move the endpoint to
				// the
				// neighbours position
				if (getNumberOfNeighbors(stack, x, y, z) == 1) {
					...
					ListIterator<int[]> iterj = this.listOfSlabVoxels
							.listIterator();
					while (iterj.hasNext()) {
						int[] slabVoxel = iterj.next();
						if (slabVoxel[0] == x && slabVoxel[1] == y
								&& slabVoxel[2] == z) {
							iterj.remove();
							break;
						}
					...
					// add it - this is a hack, logic is wrong somewhere
					// upstream
					ListIterator<int[]> iterj = this.listOfSlabVoxels
							.listIterator();
					...
					while (iterj.hasNext()) {
						int[] slab = iterj.next();
						if (slab[0] == endPoint[0] && slab[1] == endPoint[1]
								&& slab[2] == endPoint[2]) {
	...
	private boolean isJunctionMiddle(ImageStack image, int[] junctionCoord,
			int[] LUT, Skeletonize3D sk) {
		...
		// so see if it is really a branch stub and not in the middle of a
		// junction
		for (int j = 0; j < 27; j++) {
			if (neighbors[j] > 0)
				neighbors[j] = 1;
		...
		if (!sk.isEulerInvariant(neighbors, LUT)) {
			// junction voxels in the middle of a junction shouldn't contribute
			// to
			// branches
			return true;
		} else {
