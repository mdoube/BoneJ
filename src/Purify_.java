 ...
 *         14: 173-182. <a
 *         href="http://dx.doi.org/10.1016/8756-3282(93)90245-6">doi:10.1016/8756-3282(93)90245-6</a>
 *         </p>
 *         <p>
 *         Object counter 3D <a
 *         href="http://rsbweb.nih.gov/ij/plugins/track/objects.html">home page
 *         at the NIH</a>
 *         </p>
 * 
 */
public class Purify_ implements PlugInFilter {
	...
	if (nChunks > 1) {
	    ...
	    connectStructures(workArray, particleLabels, foreground, stitchRanges);
	}

	long[] particleSizes = getParticleSizes(workArray, particleLabels, foreground);
	removeSmallParticles(workArray, particleLabels,	particleSizes, foreground);
	...
	if (nChunks > 1) {
	    chunkString = ": stitching...";
	    connectStructures(workArray, particleLabels, background, stitchRanges);
	}
	...
	particleSizes = getParticleSizes(workArray, particleLabels, background);
	removeSmallParticles(workArray, particleLabels,	particleSizes, background);
	...
	if(showPerformance) showResults(chunkRanges, duration);
	return;
    }
    ...
    /**
     * Get a 2 d array that defines the z-slices to scan within while
     * connecting particles within chunkified stacks.
     * 
     * @param nC
     *                number of chunks
     * @return scanRanges int[][] containing 4 limits: int[0][] - start of
     *         outer for; int[1][] end of outer for; int[3][] start of inner
     *         for; int[4] end of inner 4. Second dimension is chunk number.
     */
    public int[][] getChunkRanges(int nC) {
	int[][] scanRanges = new int[4][nC];
	scanRanges[0][0] = 0; // the first chunk starts at the first (zeroth) slice
	scanRanges[2][0] = 0; // and that is what replaceLabel() will work on first
    ...
    /**
     * Return scan ranges for stitching. The first 2 values for each chunk
     * are the first slice of the next chunk and the last 2 values are the
     * range through which to replaceLabels()
     * 
     * Running replace labels over incrementally increasing volumes as
     * chunks are added is OK (for 1st interface connect chunks 0 & 1, for
     * 2nd connect chunks 0, 1, 2, etc.)
     * 
     * @param nC
     *                number of chunks
     * @return scanRanges list of scan limits for connectStructures() to
	...
	int[][] scanRanges = new int[4][3* (nC - 1)]; // there are nC - 1 interfaces

	for (int c = 0; c < nC - 1; c++) {
	    scanRanges[0][c] = (c + 1) * slicesPerChunk;
	    scanRanges[1][c] = (c + 1) * slicesPerChunk + 1;
	    scanRanges[2][c] = c * slicesPerChunk; //forward and reverse algorithm
//	    scanRanges[2][c] = 0; //cumulative algorithm - reliable but O^2 hard
	...
	for (int c = 0; c < scanRanges[0].length; c++) {
		...
		for (int z = sR0; z < sR1; z++) {
			...
			for (int x = 0; x < width; x++) {
				...
				for (int vZ = z - 1; vZ <= z + 1; vZ++) {
				    for (int vY = y - 1; vY <= y + 1; vY++) {
					for (int vX = x - 1; vX <= x + 1; vX++) {
					    if (withinBounds(vX, vY, vZ, sR2, sR3)) {
						int offset = getOffset(vX, vY);
						if (workArray[vZ][offset] == phase) {
						    int tagv = particleLabels[vZ][offset];
						    if (tagv != 0 && tagv < minTag) {
		...
		for (int z = sR0; z < sR1; z++) {
			...
			for (int x = 0; x < width; x++) {
				...
				for (int n = 0; n < 7; n++) {
				    switch(n){
				    case 0: nZ = z; break; //last switch block left nZ  = z + 1;
				    case 1: nX = x-1; break;
				    case 2: nX = x+1; break;
				    case 3: nY = y-1; nX = x; break;
				    case 4: nY = y+1; break;
	...
	public ConnectStructuresThread(int thread, int nThreads,
		byte[][] workArray, int[][] particleLabels, int phase, int nChunks,
		int[][] chunkRanges) {
	    this.thread = thread;
	...
	public void run() {
		...
		connectStructures(this.workArray, this.particleLabels, this.phase, singleChunkRange);
	    }
	}
    ...
    /**
     * <p>
     * Find particles of phase that touch the stack sides and assign them
     * the ID of the biggest particle of phase. Euler number calculation
     * assumes that the background phase is connected outside the image
     * stack, so apparently isolated background particles touching the sides
     * should be assigned to the single background particle.
     * </p>
     * 
     * @param workArray
     * @param particleLabels
	...
	for (y = 0; y < height; y++) {
		...
		if (workArray[z][offset] == phase
		    ...
		    replaceLabel(particleLabels, particleLabels[z][offset], biggestParticle, 0, nSlices);
		}
	    }
	}
     ...
     * @return workArray
     */
    public void removeSmallParticles(byte[][] workArray, int[][] particleLabels,
	    long[] particleSizes, int phase) {
	long maxVoxCount = 0;
	for (int i = 0; i < particleSizes.length; i++) {
	    if (particleSizes[i] > maxVoxCount) {
    ...
    /**
     * Check to see if the pixel at (m,n,o) is within the bounds of the
     * current stack
     * 
     * @param m
     *                x co-ordinate
     ...
     */
    protected void replaceLabel(int[][] particleLabels, int m, int n, int startZ, int endZ) {
