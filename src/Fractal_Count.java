 ...
 * <br />
 * Free Software in the Public domain.</p> 
 * @author Per Christian Henden
 * @author Jens Bache-Wiig
 * @author Michael Doube (refactoring)
 * @see <p><a href="http://www.pvv.org/~perchrh/imagej/fractal.html">http://www.pvv.org/~perchrh/imagej/fractal.html</a></p>
 * 
 */

public class Fractal_Count implements PlugInFilter {
				...
				for (int j = 0; j <= jMax; j += boxSize) {
					...
					for (int x = xStart; x < xEnd; x++) {
						...
						for (int z = zStart; z < zEnd; z++) {
						    ...
						    // If pixel inside region, count it

						    if ((0xff & ((byte[]) imRef.getStack().getPixels(zPos + 1))[xPos + yPart]) >= threshold) {
							count++;
							z = x = y = boxSize; // stops things
						    }
						}
					    }
					}
	...
	IJ.showMessage("About FractalCount..", "This plugin calculates the boxing dimension"
