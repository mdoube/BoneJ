...
/** ******************************************************************************************** **/
      ...
      void getRoiCoordinates(){
				...
				"\nel_size z (in mm): "+ el_size_mm_z);
                      nFirstSlice = 0; // Frage: könnte die Nullpointer exeption hier begründet sein - normal von 0 bis n-1, hier ab 1???
                      ...
                      if (nFirstSlice > 0){
                          ...
                          // kurze Bemerkung nebenbei: diese Zeile und die Variable skipSlices scheint mir eigentlich ueberfluessig !!
                       ...
                       if (zdimension > zdimension - nFirstSlice){
                          zdimension = zdimension - nFirstSlice;     // Frage: könnte die Nullpointer exeption hier begründet sein - normal von 0 bis n-1, hier ab 1???
                  }
...
public void copyISQ(String in) throws IOException{
  ...
  try {
    ...
    /** Strategie:
    anhand von offset ersten Puffer so gross wie offset wählen
    dann diesen Header patchen mit crop daten
    Diesen Header raus schreiben **/
    ...
    while (bufferCounter < 1){
         ...
         // There are two kinds of division in Java, integer and floating point. They both use the / operator to ensure you will get them confused. It depends on whether the operands surrounding it are int/long or float/double which form is used. Integer division always gives an integer result, no fraction. 
         // KH man könnte hieraus leicht eine Methode machen 
         // was ich auch noch nicht kontrolliert habe, ist die Java Besonderheit mit den signed bytes
   ...
   /** Strategie:
    ...
    dann den zweiten buffer definieren 
    und bis zum schluss dann wie gehabt abarbeiten.
    **/ 
    int count;
    ...
    while (bufferCounter <= zdimension){    
          ...
          // I copy only the content of the ROI into the buffer which has to be writen to the Output-Stream
          ...
          for (int u=0;u<heightROI;u++){      //Lines in the ROI buffer
              for (int v=0,j=0;v<widthROI;v++,j=j+2){   //columns in the ROI buffer
                  ...
                  sliceBufferROI[u*widthROI*2+j]   = sliceBuffer[startROI*2 + u*(widthROI + gapBetweenLines)*2 + j];
                  sliceBufferROI[u*widthROI*2+j+1] = sliceBuffer[startROI*2 + u*(widthROI + gapBetweenLines)*2+ j+1];
