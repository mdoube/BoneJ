 ...
 * complex numbers yields nfft complex numbers. Those complex numbers are packed
 * into arrays of floats as [real_0, imag_0, real_1, imag_1, ...]. Here, real_k
 * and imag_k correspond to the real and imaginary parts, respectively, of the
 * complex number with array index k.
 * <p>
 * When input and output arrays are the same array, transforms are performed
 * in-place. For example, an input array cx[2*nfft] of nfft complex numbers may
 * be the same as an output array cy[2*nfft] of nfft complex numbers. By
 * "the same array", we mean that cx==cy.
 * <p>
 * Transforms may be performed for any dimension of a multi-dimensional array.
 * For example, we may transform the 1st dimension of an input array
 * cx[n2][2*nfft] of n2*nfft complex numbers to an output array cy[n2][2*nfft]
 * of n2*nfft complex numbers. Or, we may transform the 2nd dimension of an
 * input array cx[nfft][2*n1] of nfft*n1 complex numbers to an output array
 * cy[nfft][2*n1] of nfft*n1 complex numbers. In either case, the input array cx
 * and the output array cy may be the same array, such that the transform may be
 * performed in-place.
 * 
...
public class FftComplex {
	...
	/**
	 * Constructs a new FFT, with specified length. Valid FFT lengths can be
	 * obtained by calling the methods {@link #nfftSmall(int)} and
	 * {@link #nfftFast(int)}.
	 * 
	 * @param nfft
	...
	public FftComplex(int nfft) {
		Check.argument(Pfacc.nfftValid(nfft), "nfft=" + nfft
				+ " is valid FFT length");
		_nfft = nfft;
	}
	...
	/**
	 * Computes a complex-to-complex fast Fourier transform. Transforms a 1-D
	 * input array cx[2*nfft] of nfft complex numbers to a 1-D output array
	 * cy[2*nfft] of nfft complex numbers.
	 * 
	 * @param sign
	 ...
	 * Transforms a 2-D input array cx[n2][2*nfft] of n2*nfft complex numbers to
	 * a 2-D output array cy[n2][2*nfft] of n2*nfft complex numbers.
	 * 
	 * @param sign
	 *            the sign (1 or -1) of the exponent used in the FFT.
	 * @param n2
	 ...
	 * Transforms a 2-D input array cx[nfft][2*n1] of nfft*n1 complex numbers to
	 * a 2-D output array cy[nfft][2*n1] of nfft*n1 complex numbers.
	 * 
	 * @param sign
	 *            the sign (1 or -1) of the exponent used in the FFT.
	 * @param n1
	 ...
	 *            the output array.
	 */
	public void complexToComplex1(int sign, int n2, int n3, float[][][] cx,
			float[][][] cy) {
		checkSign(sign);
	 ...
	 *            the output array.
	 */
	public void complexToComplex2(int sign, int n1, int n3, float[][][] cx,
			float[][][] cy) {
		checkSign(sign);
	 ...
	 *            the output array.
	 */
	public void complexToComplex3(int sign, int n1, int n2, float[][][] cx,
			float[][][] cy) {
		checkSign(sign);
	...
	/**
	 * Scales n1 complex numbers in the specified array by 1/nfft. The inverse
	 * of a complex-to-complex FFT is a complex-to-complex FFT (with opposite
	 * sign) followed by this scaling.
	 * 
	 * @param n1
	 *            1st (only) dimension of the array cx.
	 * @param cx
	 *            the input/output array[2*n1].
	 */
	...
	/**
	 * Scales n1*n2 complex numbers in the specified array by 1/nfft. The
	 * inverse of a complex-to-complex FFT is a complex-to-complex FFT (with
	 ...
	 * @param cx
	 *            the input/output array[n2][2*n1].
	 */
	...
	/**
	 * Scales n1*n2*n3 complex numbers in the specified array by 1/nfft. The
	 * inverse of a complex-to-complex FFT is a complex-to-complex FFT (with
	 * opposite sign) followed by this scaling.
	 * 
	 * @param n1
	 ...
	 * @param cx
	 *            the input/output array[n3][n2][2*n1].
	 */
	...
	private static void checkArray(int n1, int n2, int n3, float[][][] a,
			String name) {
