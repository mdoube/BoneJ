...
public class Get_Vertical_Flip implements PlugIn {
	...
	public void run(String arg) {
		...
		if (dialog.wasOKed()){ //Stop in case of cancel..
			...
			/*Use roi bones to figure out which side the soft tissue lies compared to the line connecting the bones...*/
			SoftTissueSide softTissueSide = new SoftTissueSide((SelectROI) roi,imageAndAnalysisDetails,changeSign);
			...
			if(suppressImages && !saveImageOnDisk && roi != null){
				...
				addSoftTissueSideSieve(resultImage,softTissueSide.softTissueSideSieve);
			}
	...
	ImagePlus getRGBResultImage(double[] values,int width,int height){
		...
		/*Add soft side sieve*/
	ImagePlus addSoftTissueSideSieve(ImagePlus tempImage, byte[] sieve){
		for (int y = 0; y < tempImage.getHeight();++y) {
			for (int x = 0; x < tempImage.getWidth();++x) {
				if (sieve[x+y*tempImage.getWidth()] == 1){   //Tint other side with blue
				...
				if (sieve[x+y*tempImage.getWidth()] == 2){   //Tint other side with red
					int value = tempImage.getProcessor().getPixel(x,y);
					int[] rgb = new int[3];
					for (int i = 0; i<3;++i){
						rgb[i] = (value >>(i*8))& 0XFF;
				...
				if (sieve[x+y*tempImage.getWidth()] == 3){   //Tint other side with red
					int value = tempImage.getProcessor().getPixel(x,y);
					int[] rgb = new int[3];
					for (int i = 0; i<3;++i){
						rgb[i] = (value >>(i*8))& 0XFF;
				...
				if (sieve[x+y*tempImage.getWidth()] == 4){   //Tint other side with red
					int value = tempImage.getProcessor().getPixel(x,y);
					int[] rgb = new int[3];
					for (int i = 0; i<3;++i){
						rgb[i] = (value >>(i*8))& 0XFF;
