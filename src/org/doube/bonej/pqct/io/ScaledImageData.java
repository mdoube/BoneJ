...
public class ScaledImageData{
	...
	//Constructor STRATEC
	public ScaledImageData(ReadStratecFile data, ImageAndAnalysisDetails detailsIn){
		ImageAndAnalysisDetails details =detailsIn;
		height = data.PicMatrixY;
		width = data.PicMatrixX;
		pixelSpacing = data.VoxelSize;
		filterSize = details.filterSize;		//filterSize x filterSize median filter will be used
		double[] unFiltered = new double[width*height];
		minimum = 0;	//Save minimum value
		maximum = 0;	//Save maximum value
		for (int t = 0;t<width*height;t++){	//Scale the image
			unFiltered[t] = ((double) data.data[t])*details.scalingFactor+details.constant;
			if (unFiltered[t] < minimum) {minimum = unFiltered[t];}
			if (unFiltered[t] > maximum) {maximum = unFiltered[t];}
		}
		double[] unFilteredSoft = (double[])unFiltered.clone();			//For 7x7 median filtering...	
		scaledImage = medianFilter(unFiltered,width,height,filterSize); //Median filter data
		softScaledImage = medianFilter(unFilteredSoft,width,height,details.softFilterSize); //Median filter data
		//System.out.println("Data skaalattu kahdella eri filtterilla");
	}
	
	//Constructor DICOM
	public ScaledImageData(DicomReader data, ImageAndAnalysisDetails detailsIn){
		ImageAndAnalysisDetails details =detailsIn;
		height = data.rows;
		width = data.columns;
		String pSpace =new String(data.pixelSpacing);
		pixelSpacing = Double.parseDouble(pSpace.substring(0,pSpace.indexOf(new String("\\"))-1));
		filterSize = details.filterSize;		//filterSize x filterSize median filter will be used
		double[] unFiltered = new double[width*height];
		minimum = 0;	//Save minimum value
		maximum = 0;	//Save maximum value
		for (int t = 0;t<width*height;t++){	//Scale the image
			//if (t > width*height*2/3){
				unFiltered[t] = ((double) data.data[t])*details.scalingFactor+details.constant;
				if (unFiltered[t] < minimum) {minimum = unFiltered[t];}
				if (unFiltered[t] > maximum) {maximum = unFiltered[t];}
			//}else {
	...
	//Constructor Papyrus DICOM
	public ScaledImageData(DicomFile data, ImageAndAnalysisDetails detailsIn){
		ImageAndAnalysisDetails details =detailsIn;
		height = data.rows;
		width = data.columns;
		String pSpace =new String(data.pixelSpacing);
		...
		for (int t = 0;t<width*height;t++){	//Scale the image
			//if (t > width*height*2/3){
				unFiltered[t] = ((double) data.data2[t])*details.scalingFactor+details.constant;
				if (unFiltered[t] < minimum) {minimum = unFiltered[t];}
				if (unFiltered[t] > maximum) {maximum = unFiltered[t];}
			//}else {
			//	unFiltered[t]=0;
