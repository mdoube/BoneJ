...
public class BinaryInterpolator {
	...
	public void run(final ImagePlus image, final Roi[] rois) {
		w = image.getWidth();
		h = image.getHeight();
		...
		run(stack);
		final ImagePlus roiImage = new ImagePlus("bla", stack);

		final ThresholdToSelection ts = new ThresholdToSelection();
		ts.setup("", roiImage);
		for (int i = firstIndex; i <= lastIndex; i++) {
			final ImageProcessor bp = stack.getProcessor(1);
			stack.deleteSlice(1);
	...
	public void run(final ImageStack stack) {
		...
		for (int z = first; z < last; z++) {
			...
			final byte[] p = (byte[]) stack.getProcessor(z + 1).getPixels();
			for (int i = 0; i < w * h; i++)
	...
	/*
	 * The following calculates the signed integer distance transform. Distance
	 * transform means that each pixel is assigned the distance to the boundary.
	 * IDT means that the distance is not the Euclidean, but the minimal sum of
	 * neighbour distances with 3 for horizontal and neighbours, and 4 for
	 * diagonal neighbours (in 3d, the 3d diagonal neighbour would be 5). Signed
	 * means that the outside pixels have a negative sign.
	 */
	class IDT {
		int[] result;
		...
		final void idt(final int x, final int y, final int dx, final int dy) {
	...
	int[] getIDT(final Object pixels) {
		final IDT idt = new IDT();
	...
	final boolean isBoundary(final byte[] pixels, final int x, final int y) {
		if (pixels[x + w * y] == 0)
			return false;
		...
		if (x >= w - 1 || y <= 0 || pixels[x + 1 + w * (y - 1)] == 0)
			return true;
		if (x >= w - 1 || y >= h - 1 || pixels[x + 1 + w * (y + 1)] == 0)
			return true;
		return false;
	}

	final boolean isJustOutside(final byte[] pixels, final int x, final int y) {
		if (pixels[x + w * y] != 0)
			return false;
		if (x > 0 && pixels[x - 1 + w * y] != 0)
			return true;
