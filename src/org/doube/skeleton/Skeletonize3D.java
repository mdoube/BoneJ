...
public class Skeletonize3D implements PlugIn {
	 ...
	 *            output image stack
	 */
	private ImagePlus computeThinImage(ImageStack outputImage) {
		...
		// border types
		{
			...
			for (int currentBorder = 1; currentBorder <= 6; currentBorder++) {
				IJ.showStatus("Thinning iteration " + iter + " ("
						+ currentBorder + "/6 borders) ...");
				...
				for (int z = 0; z < d; z++) {
					for (int y = 0; y < h; y++) {
						for (int x = 0; x < w; x++) {
							...
							// North
							if (currentBorder == 1
									&& N(outputImage, x, y, z, w, h, d) <= 0)
								isBorderPoint = true;
							// South
							if (currentBorder == 2
									&& S(outputImage, x, y, z, w, h, d) <= 0)
								isBorderPoint = true;
							// East
							if (currentBorder == 3
									&& E(outputImage, x, y, z, w, h, d) <= 0)
								isBorderPoint = true;
							// West
							if (currentBorder == 4
									&& W(outputImage, x, y, z, w, h, d) <= 0)
								isBorderPoint = true;
							// Up
							if (currentBorder == 5
									&& U(outputImage, x, y, z, w, h, d) <= 0)
								isBorderPoint = true;
							// Bottom
							if (currentBorder == 6
									&& B(outputImage, x, y, z, w, h, d) <= 0)
								isBorderPoint = true;
							...
							// be counted as well
							byte[] neighbor = getNeighborhood(outputImage, x,
									y, z, w, h, d);
							...
							if (!isEulerInvariant(getNeighborhood(outputImage,
									x, y, z, w, h, d), eulerLUT)) {
								continue; // current point is not deletable
							}
							// Check if point is simple (deletion does not
							// change connectivity in the 3x3x3 neighborhood)
							if (!isSimplePoint(getNeighborhood(outputImage, x,
									y, z, w, h, d))) {
								continue; // current point is not deletable
							}
