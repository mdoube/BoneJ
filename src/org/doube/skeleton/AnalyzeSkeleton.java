 ...
 * images.
 * <p>
 * For more detailed information, visit the AnalyzeSkeleton home page: <A
 * target="_blank"
...
public class AnalyzeSkeleton implements PlugInFilter {
	...
	public static final String[] pruneCyclesModes = { "none",
			"shortest branch", "lowest intensity voxel",
	...
	public SkeletonResult run(int pruneIndex, boolean pruneEnds, ImagePlus skeleton,
		...
		if (bPruneCycles) {
			if (pruneCycles(this.inputImage, this.originalImage,
					AnalyzeSkeleton.pruneIndex)) {
				// initialize visit flags
				resetVisited();
	...
	private void calculateNeighborhoodOffsets(Calibration calibration) {
		...
		this.x_offset = ((int) Math.round(max / calibration.pixelWidth) > 1) ? (int) Math
				.round(max / calibration.pixelWidth)
				: 1;
		this.y_offset = ((int) Math.round(max / calibration.pixelHeight) > 1) ? (int) Math
				.round(max / calibration.pixelHeight)
				: 1;
		this.z_offset = ((int) Math.round(max / calibration.pixelDepth) > 1) ? (int) Math
				.round(max / calibration.pixelDepth)
				: 1;
	...
	public void processSkeleton(ImageStack inputImage2) {
		...
		if (this.numOfTrees > 1)
			...
			if (debug)
				IJ.log("list of end points size = "
						+ this.listOfEndPoints.size());
	...
	private boolean pruneCycles(ImageStack inputImage,
			final ImageStack originalImage, final int pruningMode) {
		...
		for (int iTree = 0; iTree < this.numOfTrees; iTree++) {
			...
			if (this.startingSlabTree[iTree].size() == 1) {
				setPixel(inputImage, this.startingSlabTree[iTree].get(0),
						(byte) 0);
				pruned = true;
				...
				if (backEdges.size() > 0) {
					...
					for (final Edge e : backEdges) {
						...
						final Vertex finalLoopVertex = e.getV1()
								.getVisitOrder() < e.getV2().getVisitOrder() ? e
								.getV1()
								: e.getV2();

						Vertex backtrackVertex = e.getV1().getVisitOrder() < e
								.getV2().getVisitOrder() ? e.getV2() : e
								.getV1();
						...
						if (pruningMode == AnalyzeSkeleton.SHORTEST_BRANCH) {
							...
							if (minEdge.getSlabs().size() > 0)
								removeCoords = minEdge.getSlabs().get(
										minEdge.getSlabs().size() / 2);
							else
								removeCoords = minEdge.getV1().getPoints().get(
										0);
	 ...
	 *            original gray image
	 */
	private void removeLowestIntensityVoxel(final ArrayList<Edge> loopEdges,
			ImageStack inputImage2, ImageStack originalGrayImage) {
		...
		for (final Edge e : loopEdges) {
			...
			for (final Point p : e.getSlabs()) {
				final double avg = getAverageNeighborhoodValue(
						originalGrayImage, p, this.x_offset, this.y_offset,
						this.z_offset);
	 ...
	 *            input skeleton image
	 */
	private void cutLowestIntensityBranch(final ArrayList<Edge> loopEdges,
			ImageStack inputImage2, ImageStack originalGrayImage) {
		...
		for (final Edge e : loopEdges) {
			...
			for (final Point p : e.getV1().getPoints()) {
				edgeIntensity += getAverageNeighborhoodValue(originalGrayImage,
						p, this.x_offset, this.y_offset, this.z_offset);
				n_vox++;
			}
	...
	private void divideVoxelsByTrees(ImageStack treeIS) {
		...
		for (int iTree = 0; iTree < this.numOfTrees; iTree++) {
			...
			this.numberOfJunctionVoxels[iTree] = this.junctionVoxelTree[iTree]
					.size();
		}
	...
	private void showResults() {
		...
		final String[] head = { "Skeleton", "# Branches", "# Junctions",
				...
				"Average Branch Length", "# Triple points",
	...
	private void visitSkeleton(ImageStack taggedImage, ImageStack treeImage,
		...
		if (debug) {
			...
			IJ.log("this.junctionVertex[" + (iTree) + "].length = "
					+ this.junctionVertex[iTree].length);
		...
		for (int i = 0; i < this.numberOfEndPoints[iTree]; i++) {
			...
			this.graph[iTree].addEdge(new Edge(v1, this.auxFinalVertex,
		...
		if (this.numberOfEndPoints[iTree] == 0
				&& this.junctionVoxelTree[iTree].size() > 0)
		...
		// 08/26/2009 Changed the loop to visit first the junction voxel that
		// are
		// forming a single junction.
		for (int i = 0; i < this.junctionVertex[iTree].length; i++) {
			for (int j = 0; j < this.junctionVertex[iTree][i].getPoints()
					.size(); j++) {
				final Point junctionCoord = this.junctionVertex[iTree][i]
						.getPoints().get(j);
					...
					if (!isJunction(nextPoint)) {
						...
						if (length != 0) {
							...
							for (int k = 0; k < this.junctionVertex[iTree].length; k++)
								if (this.junctionVertex[iTree][k]
							...
							if (isSlab(this.auxPoint)) {
								...
								// IJ.log("Looking for " + this.auxPoint +
								// " in the list of vertices...");
								this.auxPoint = getVisitedJunctionNeighbor(
										this.auxPoint, initialVertex);
								this.auxFinalVertex = findPointVertex(
										this.junctionVertex[iTree],
										this.auxPoint);
								if (this.auxPoint == null) {
									// IJ.error("Point "+ aux +
									// " has not neighbor end junction!");
									// Inner loop
							...
							if (debug)
								IJ
										.log("increased number of branches, length = "
												+ length
												+ " (last point = "
												+ this.auxPoint + ")");
							// update maximum branch length
							...
							if (debug)
								IJ.log("adding branch from "
										...
										+ " to "
										+ this.auxFinalVertex.getPoints()
												.get(0));
							this.graph[iTree]
									.addEdge(new Edge(initialVertex,
											this.auxFinalVertex, this.slabList,
											length));
						}
					} else
		...
		if (this.numberOfBranches[iTree] == 0)
			return;
		// Average length
		this.averageBranchLength[iTree] = branchLength
				/ this.numberOfBranches[iTree];

		if (debug) {
			IJ.log("Num of vertices = "
					+ this.graph[iTree].getVertices().size()
					+ " num of edges = " + this.graph[iTree].getEdges().size());
			for (int i = 0; i < this.graph[iTree].getVertices().size(); i++) {
				...
				for (int j = 0; j < v.getBranches().size(); j++) {
					...
					final Vertex oppositeVertex = v1.equals(v) ? v
							.getBranches().get(j).getV2() : v1;
			...
			for (int i = 0; i < this.junctionVertex[iTree].length; i++) {
				...
				for (int j = 0; j < this.junctionVertex[iTree][i].getPoints()
						.size(); j++)
					IJ.log(j + ": "
							+ this.junctionVertex[iTree][i].getPoints().get(j));
			}
		}
	...
	private ImageStack markTrees(ImageStack taggedImage) {
		...
		ImageStack outputImage = new ImageStack(this.width, this.height,
				taggedImage.getColorModel());
		for (int z = 0; z < depth; z++) {
			outputImage.addSlice(taggedImage.getSliceLabel(z + 1),
					new ShortProcessor(this.width, this.height));
		}
	...
	private double visitBranch(Point startingPoint, int iTree) {
		...
		if (nextPoint != null) {
			...
			if (isEndPoint(nextPoint)) {
				...
				this.auxFinalVertex = findPointVertex(
						this.junctionVertex[iTree], nextPoint);
				/*
				 ...
				 * IJ.log(" " + nextPoint + " belongs to junction " +
	...
	private double calculateDistance(Point point1, Point point2) {
		return Math.sqrt(Math.pow((point1.x - point2.x)
				* this.imRef.getCalibration().pixelWidth, 2)
				+ Math.pow((point1.y - point2.y)
						* this.imRef.getCalibration().pixelHeight, 2)
				+ Math.pow((point1.z - point2.z)
						* this.imRef.getCalibration().pixelDepth, 2));
	}
	...
	private void groupJunctions(ImageStack treeIS) {
		...
		for (int iTree = 0; iTree < this.numOfTrees; iTree++) {
			...
			for (int i = 0; i < this.numberOfJunctionVoxels[iTree]; i++) {
				...
				if (!isVisited(pi))
					fusionNeighborJunction(pi,
							this.listOfSingleJunctions[iTree]);
			}
		}
		...
		for (int iTree = 0; iTree < this.numOfTrees; iTree++) {
			...
			this.numberOfJunctions[iTree] = this.listOfSingleJunctions[iTree]
					.size();

			// Create array of junction vertices for the graph
			this.junctionVertex[iTree] = new Vertex[this.listOfSingleJunctions[iTree]
					.size()];

			for (int j = 0; j < this.listOfSingleJunctions[iTree].size(); j++) {
				final ArrayList<Point> list = this.listOfSingleJunctions[iTree]
						.get(j);
	...
	private void calculateTripleAndQuadruplePoints() {
		for (int iTree = 0; iTree < this.numOfTrees; iTree++) {
			...
			for (int i = 0; i < this.numberOfJunctions[iTree]; i++) {
				...
				ArrayList<Point> groupOfJunctions = this.listOfSingleJunctions[iTree]
						.get(i);
				...
				for (int j = 0; j < groupOfJunctions.size(); j++) {
					...
					byte[] neighborhood = this.getNeighborhood(
							this.taggedImage, pj.x, pj.y, pj.z);
					for (int k = 0; k < 27; k++)
						if (neighborhood[k] == AnalyzeSkeleton.SLAB
								|| neighborhood[k] == AnalyzeSkeleton.END_POINT)
							nBranch++;
				}
		...
		for (int x = -1; x < 2; x++)
			for (int y = -1; y < 2; y++)
				for (int z = -1; z < 2; z++) {
					...
					if (getPixel(this.inputImage, point.x + x, point.y + y,
							point.z + z) != 0
	...
	public static double getAverageNeighborhoodValue(final ImageStack image,
			final Point p, final int x_offset, final int y_offset,
			final int z_offset) {
	 ...
	 */
	public static byte[] getNeighborhood(final ImageStack image, final Point p,
	 ...
	 */
	void showAbout() {
		IJ.showMessage("About AnalyzeSkeleton...",
