...
public class RoiMan {
	 ...
	 * @return
	 */
	public static ArrayList<Roi> getSliceRoi(final RoiManager roiMan, final int s) {
	 ...
	 */
	public static int[] getLimits(final RoiManager roiMan) {
		...
		if (noZroi) {
			final int[] limits = { xmin, xmax, ymin, ymax, 1, Integer.MAX_VALUE };
			return limits;
		} else {
	 ...
	 */
	public static ImageStack cropStack(final RoiManager roiMan, final ImageStack stack, final boolean fillBackground,
			final int fillValue, final int padding) {
		final int[] limits = getLimits(roiMan);
		final int xmin = limits[0];
		final int xmax = limits[1];
		final int ymin = limits[2];
		final int ymax = limits[3];
		final int zmin = limits[4];
		final int zmax = (limits[5] == Integer.MAX_VALUE) ? stack.getSize() : limits[5];
		...
		final ImageStack out = new ImageStack(w, h);
		for (int z = 1; z <= d; z++) {
			final ImageProcessor ip = imp.getProcessor().createProcessor(w, h);
			...
			final ImageProcessor ipSource = stack.getProcessor(z - zOff);
			if (fillBackground)
				for (int i = 0; i < length; i++)
					ip.set(i, fillValue);
			final ArrayList<Roi> rois = getSliceRoi(roiMan, z - zOff);
			for (final Roi roi : rois) {
				ipSource.setRoi(roi);
				final Rectangle r = roi.getBounds();
				final ImageProcessor mask = ipSource.getMask();
