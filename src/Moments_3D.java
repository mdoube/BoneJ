...
public class Moments_3D implements PlugInFilter {
	...
	public void run(ImageProcessor ip) {
    	...
    	EigenvalueDecomposition E = calculateMoments(stack, startSlice, endSlice, centroid, rt);
    	rt.show("Results");
    	if (doAlign) alignToPrincipalAxes(stack, E, centroid);
    	return;
	}
	...
	/*-----------------------------------------------------------------*/
	/**
	 * Convert a pixel value <i>x</i> to a voxel density <i>y</i> given calibration constants <i>m</i>, <i>c</i>
	 * for the equation <i>y</i> = <i>mx</i> + <i>c</i>
	...
	public double voxelDensity(double pixelValue, double m, double c){
		...
		if (voxelDensity < 0) voxelDensity = 0;
		return voxelDensity;
	}/*end voxelDensity */
	
	/*------------------------------------------------------------------*/
	/**
	 * Find the centroid of an ImageStack using startSlice and endSlice as
	 * top and bottom limits and a Rectangle as x and y limits.
	 * 
	 * @param stack
	 * @param startSlice
	 * @param endSlice
	...
	public double[] findCentroid3D(ImageStack stack, int startSlice, int endSlice, ResultsTable rt){
	    ...
	    int sliceSize = w*h;   	
    	double sumx = 0; double sumy = 0; double sumz = 0; double sumMass = 0;
	    for (int z = startSlice; z <= endSlice; z++){
	    	...
	    	for (int y = r.y; y < (r.y+r.height); y++) {
		        ...
		        for (int x=r.x; x<(r.x+r.width); x++) {
		            i = offset + x;
		            int testPixel = slicePixels[i]&0xffff;  //convert signed short to int
		            if (testPixel >= minT && testPixel <= maxT){
		            	double voxelMass = voxelDensity(testPixel, m, c) * voxVol;
		            	sumMass += voxelMass;
...
//	    for (int n = 0; n<sourceWorkArray.length; n++) sourceWorkArray[n] = 0;
	...
	public EigenvalueDecomposition calculateMoments(ImageStack stack, int startSlice, int endSlice, double[] centroid, ResultsTable rt){
	    	...
	    	for (int y=r.y; y < (r.y+r.height); y++) {
	    		...
	    		for (int x=r.x; x<(r.x+r.width); x++) {
	    			...
	    			if (testPixel >= minT && testPixel <= maxT){
	    				...
	    				sumVoxMass += voxMass;
	    				Icxx += ((y*vH-cY)*(y*vH-cY) + (z*vD-cZ)*(z*vD-cZ) + (vH*vH + vD*vD) / 12) * voxMass;
	    				Icyy += ((x*vW-cX)*(x*vW-cX) + (z*vD-cZ)*(z*vD-cZ) + (vW*vW + vD*vD) / 12) * voxMass;
	    				Iczz += ((y*vH-cY)*(y*vH-cY) + (x*vW-cX)*(x*vW-cX) + (vH*vH + vW*vW) / 12) * voxMass;
	...
	public void alignToPrincipalAxes(ImageStack stack, EigenvalueDecomposition E, double[] centroid){
		...
		int w = stack.getWidth();
		int sliceSize = w*h;
		short[] sourceWorkArray = makeWorkArray(stack, startSlice, endSlice);
...
//		for each voxel in the target stack, find the corresponding source voxel
		for (int z = 1; z < al; z++){
			...
			for (int y = 0; y < h; y++){
				for (int x = 0; x < w; x++){
					double xD = x * vW - centroid[0];
					double yD = y * vH - centroid[1];
					double zD = z * vD - centroid[2];
					double xAlign = xD*eigenVectorsInverse[2][0] + yD*eigenVectorsInverse[1][0] + zD*eigenVectorsInverse[0][0] + centroid[0];
						double yAlign = xD*eigenVectorsInverse[2][1] + yD*eigenVectorsInverse[1][1] + zD*eigenVectorsInverse[0][1] + centroid[1];
						double zAlign = xD*eigenVectorsInverse[2][2] + yD*eigenVectorsInverse[1][2] + zD*eigenVectorsInverse[0][2] + centroid[2];
						...
						if (xA >=0 && xA < w && yA >= 0 && yA < h && zA >= 0 && zA < al){
